...
| `decisions.md` | CLI (`brief sync`) | Generated from Nomo/manual |
...
## 🔴 URGENT
<!-- expires: 2026-03-31 -->
- CIMB compliance deadline moved to Friday. Kevin: drop Elwyn, focus on Nora RLS.
  Source: override (Aldo, 2026-03-29 14:15)

## NOW
- PR #52 on nora — approved, ready to merge [status: active]
  Source: scope (score: 38)
  Ref: https://github.com/While-True-Lab/nora/pull/52
- Issue #45 on nora — CRITICAL RLS policies [status: active]
  Source: scope (score: 39)
  Ref: https://github.com/While-True-Lab/nora/issues/45

## TODAY
- Elwyn PR #117-119 — sequential merge needed [status: active]
  Source: scope
- AutoDiterima #93 — CF Pages env var fix, 5 min [status: active]
  Source: github
...
## 2026-03-29
- **CIMB compliance deadline moved to Friday** — reprioritize all compliance work
  Meeting: Dearezt x WTL (Nomo)
  Impact: Kevin reassigned from Elwyn to Nora

## 2026-03-28
- **Auth approach: JWT + refresh tokens** — replaces session-based auth
  Meeting: Internal standup (Nomo)
  Impact: Elwyn LTI integration affected
```
...
## Current Focus
- Nora RLS compliance (reassigned 2026-03-29)

## Assigned
- Issue #45 on nora — CRITICAL RLS policies
- PR #52 on nora — needs merge

## Blocked On
- AutoDiterima CI (#93) — needs CF Pages env var fix from Aldo

## Recently Completed
- Elwyn LTI PR #117 (2026-03-28)
```
...
## Add
- Kevin: drop Elwyn, focus on Nora RLS compliance this week
  Reason: CIMB deadline moved (2026-03-29 meeting)
  Expires: 2026-04-04

## Remove
- issue:autoditerima#63  # parked, not actionable
- issue:autoditerima#68  # parked pending product decision

## Boost
- pr:nora#52  # approved, needs merge urgently
  Priority: now
```
...
- #87 — Security headers for LTI endpoints [P1, assigned: Kevin]
...
## Blockers
- CIMB deployment blocked on env var config (#93 on AutoDiterima)

## Recent Activity
- 2026-03-28: Scout created PR #94 (sidebar nav)
- 2026-03-27: Kevin merged PR #115 (auth fix)
```
...
## 2026-03-29
- 14:32 UTC | Scout | Read priorities.md (hash: abc123) | Created PR #94 on autoditerima | Reason: highest-scored buildable issue
- 18:00 UTC | Scout | Read priorities.md (hash: abc123) | Reviewed 24 PRs | Reason: nightly review cycle
```
...
~/wtl-brief/.brief/                # Team context (central repo)
~/projects/aivident/.brief/        # Project context (per-repo)
```
...
name = "nomo"
type = "command"
command = "mcporter call nomomeet.list_action_items status=pending --output json"
target = "decisions"
priority = 60
timeout = 15
...
nomo: last_success=2026-03-29T10:00:00Z status=stale items=5
```
...
Source: nomo       last sync: 3d ago    status: ⚠️ stale   items: 5

Files:
  priorities.md    updated: 2h ago     ✅
  decisions.md     updated: 3d ago     ⚠️ stale (threshold: 4h)
  state/elwyn.md   updated: 2h ago     ✅
```
...
3. Week 3: Add Nomo, auto-generate decisions
